...
# Integration test: ./dotfiles.sh arch_xfce4 bootstrap in a throwaway archlinux container.
...
IMAGE="${IMAGE:-dotfiles-test-arch-xfce4}"
...
echo "==> ./dotfiles.sh arch_xfce4 bootstrap"
docker run --rm \
    --platform "$DOCKER_PLATFORM" \
